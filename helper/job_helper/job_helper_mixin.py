...
class HelperMixin:
    def get_config(self, path='config/config.yaml', location='local') -> dict:
        """Reads yaml config.
        :param path: path(key)
        :type path: str
    ...
    def to_csv_and_load(self, df: pd.DataFrame, file_name: str) -> None:
    ...
    def add_ts_col_to_df(self, df: pd.DataFrame) -> pd.DataFrame:
